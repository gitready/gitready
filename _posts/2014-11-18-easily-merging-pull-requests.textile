...
Fortunately, git is easily extensible. You can add subcommands to git by having executables following the appropriate naming convention @git-subcommand@, and placing them in your @PATH@. So, for instance, whenever you call @git hello@, git will look for an executable called @git-hello@ in your @PATH@.
...
[[ $(git branchname) == $destiny ]] || exit "failed to switch to $current"
git rebase
...
So by calling @git cpr@ with no arguments, I'm mergin with @master@, but I can override that by calling @git cpr other-branch@
...
After that, I want to rebase the target branch against master. Here, I want to do an interactive rebase, since I usually want to squash or reword some commits.
If there are conflicts here, the rebase itself will abort and prevent the rest of the script from going on. If that happens, I just have to solve the conflicts like I normally would, and then try the command again.

h3. 3. Merge

The third step involves going back to master and merge the branch. There is no rocket science here. I used the @--ff-only@ option just to make sure I'm not creating a merge commit. That shouldn't happen anyway since I'm rebasing both branches prior to the merge, but you can never be too safe.

h3. 4. Delete the feature branch
...
I start by moving to another branch (to prevent problems when I trying to delete the branch I'm currently in). Then I just call the commands to delete both the remote branch on GitHub, and the local one. If you're not familiar with the syntax for the first one, @git push remote :branch@, the colon is telling git to delete the branch on the remote. I know, weird right?
...
There are a lot of suggestion out there on how to extend git, but a few suggestions would be "git-wtf":https://gitorious.org/willgit/mainline/source/a84bba3726a19bc78086852d54bb0219ea1bb6f9:bin/git-wtf and "git-publish-branch":https://gitorious.org/willgit/mainline/source/a84bba3726a19bc78086852d54bb0219ea1bb6f9:bin/git-publish-branch#L6 (although the latter is not that much useful since the latest git updates).

Additionally, I suggest you check out "hub":https://hub.github.com/, or the newer and faster alternative, "gh":http://owenou.com/gh/. Both of these add extra functionality that might prevent you from having to write your own custom scripts.
